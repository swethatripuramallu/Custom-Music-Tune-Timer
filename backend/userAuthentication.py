...
# When you deploy this application make sure to copy over your
...
REDIRECT_URL = '5460d929319845f19937d7441ad4ee51'
...
def index():
    return "Welcome to my Tune Timer <a href='/login'>Login with Spotify</a>"
    # planning on making this separate
    # where it includes a react front end page for this
...
def login():
    ...
    params = {
        ...
        'show_dialog': True
        # Force the user to login everytime into application by
        # setting show_dialog true
        # to make debugging easier for local server purpose, can
        # change at the end
    }
...
def get_playlists():
    ...
    if datetime.now().timestamp() > session['expires_at']:
        return redirect('/refresh-token')  # automatically refresh it
        # for them so we don't interrupt the user interface
...
def refresh_token():
    ...
    expin = 'expires_in'
    session['expires_at'] = datetime.now().timestamp()+new_token_info[expin]
